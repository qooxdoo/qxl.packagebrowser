    ...
    */
    ...
    members: {
      ...
      _applyModel: function _applyModel(value, old) {
        // for every target
        for (var i = 0; i < this.__P_262_1.length; i++) {
          // get the properties
          var targetObject = this.__P_262_1[i][0];
          var targetProperty = this.__P_262_1[i][1];
          var sourceProperty = this.__P_262_1[i][2];
          var bidirectional = this.__P_262_1[i][3];
          var options = this.__P_262_1[i][4];
          var reverseOptions = this.__P_262_1[i][5];

          // remove it from the old if possible
          if (old != undefined && !old.isDisposed()) {
            this.__P_262_2(targetObject, targetProperty, sourceProperty, old);
          }

          // add it to the new if available
          if (value != undefined) {
            this.__P_262_3(targetObject, targetProperty, sourceProperty, bidirectional, options, reverseOptions);
          } else {
      ...
      addTarget: function addTarget(targetObject, targetProperty, sourceProperty, bidirectional, options, reverseOptions) {
        ...
        this.__P_262_1.push([targetObject, targetProperty, sourceProperty, bidirectional, options, reverseOptions]);
       ...
       *   {@link qx.data.SingleValueBinding} class.
       */
      __P_262_3: function __P_262_3(targetObject, targetProperty, sourceProperty, bidirectional, options, reverseOptions) {
        ...
        if (this.__P_262_0[targetHash] == undefined) {
          this.__P_262_0[targetHash] = [];
        }
        this.__P_262_0[targetHash].push([id, idReverse, targetProperty, sourceProperty, options, reverseOptions]);
      },
      /**
      ...
      removeTarget: function removeTarget(targetObject, targetProperty, sourceProperty) {
        this.__P_262_2(targetObject, targetProperty, sourceProperty, this.getModel());

        // delete the target in the targets reference
        for (var i = 0; i < this.__P_262_1.length; i++) {
          if (this.__P_262_1[i][0] == targetObject && this.__P_262_1[i][1] == targetProperty && this.__P_262_1[i][2] == sourceProperty) {
            this.__P_262_1.splice(i, 1);
          }
        }
      },
      /**
       ...
       *   comes.
       */
      __P_262_2: function __P_262_2(targetObject, targetProperty, sourceProperty, sourceObject) {
        // check for not fitting targetObjects
        if (!(targetObject instanceof qx.core.Object)) {
          // just do nothing
          return;
        }
        var currentListing = this.__P_262_0[targetObject.toHashCode()];
