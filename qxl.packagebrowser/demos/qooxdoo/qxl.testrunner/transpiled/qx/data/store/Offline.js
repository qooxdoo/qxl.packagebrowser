  ...
  ************************************************************************ */
  ...
  qx.Class.define("qx.data.store.Offline", {
    ...
    construct: function construct(key, storage, delegate) {
      ...
      this._storeModel = qx.util.Function.debounce(this.__P_271_0.bind(this), qx.data.store.Offline.STORE_MODEL_DELAY);
    ...
    members: {
      ...
      _applyModel: function _applyModel(value, old) {
        ...
        if (old) {
          old.removeListenerById(this.__P_271_1);
          old.dispose();
          this.__P_271_1 = null;
        }
        if (value) {
          this.__P_271_1 = value.addListener("changeBubble", this._storeModel, this);
          this._storeModel();
        } else {
