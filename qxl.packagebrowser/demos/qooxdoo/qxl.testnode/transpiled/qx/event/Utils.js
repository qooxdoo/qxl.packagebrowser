  ...
  ************************************************************************ */
  ...
  qx.Class.define("qx.event.Utils", {
    ...
    statics: {
        ...
        true(tracker, fn) {
          ...
          if (tracker.promise) {
            if (qx.lang.Type.isPromise(fn)) {
              this.__P_42_0(tracker, tracker.promise.then(fn));
            } else {
              var self = this;
              this.__P_42_0(tracker, tracker.promise.then(function (result) {
                if (tracker.rejected) {
                  return null;
                }
                result = fn(result);
            ...
            this.__P_42_1(tracker);
            return tracker.promise;
          }
          if (qx.lang.Type.isPromise(fn)) {
            return this.__P_42_2(tracker, fn);
          }
          var result = fn(tracker.result);
          if (qx.lang.Type.isPromise(result)) {
            return this.__P_42_2(tracker, result);
          }
       ...
       * @return {qx.Promise} the new promise
       */
      __P_42_2(tracker, newPromise) {
        if (tracker.promise) {
          this.__P_42_0(tracker, tracker.promise.then(function () {
            return newPromise;
          }));
        } else {
          this.__P_42_0(tracker, newPromise);
        }
        this.__P_42_1(tracker);
        return tracker.promise;
      },
      ...
      reject(tracker) {
        ...
        var result = this.__P_42_3(tracker);
      ...
      __P_42_1(tracker) {
        if (tracker.promise && tracker.catch) {
          if (!tracker.promise["qx.event.Utils.hasCatcher"]) {
            this.__P_42_0(tracker, tracker.promise.catch(this.__P_42_3.bind(this, tracker)));
