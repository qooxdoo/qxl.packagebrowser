    ...
    statics: {
      ...
      */
      ...
      define: function (name, config) {
        ...
        config.include = this.__P_97_0(config.include);
        config.patch = this.__P_97_0(config.patch); // Validate incoming data
        ...
        this.__P_97_1(theme, config);

        this.__P_97_2(theme, config); // Store class reference in global class registry
       ...
       */
      __P_97_0: function (objectOrArray) {
        if (!objectOrArray) {
          return [];
        }
       ...
       * @param config {Map} config structure
       */
      __P_97_2: function (theme, config) {
      ...
      */
       ...
       */
      __P_97_3: function (config) {
        for (var i = 0, keys = this.__P_97_4, l = keys.length; i < l; i++) {
          if (config[keys[i]]) {
            return keys[i];
          }
        }
       ...
       */
      __P_97_1: function (theme, config) {
        var type = this.__P_97_3(config); // Use theme key from extended theme if own one is not available
      ...
      __P_97_4: ["colors", "borders", "decorations", "fonts", "icons", "widgets", "appearances", "meta"],
      ...
      patch: function (theme, mixinTheme) {
        this.__P_97_8(mixinTheme);

        var type = this.__P_97_3(mixinTheme);

        if (type !== this.__P_97_3(theme)) {
...
//# sourceMappingURL=Theme.js.map?dt=1626058137381