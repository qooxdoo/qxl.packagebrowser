(function () {
  ...
  ************************************************************************ */
  ...
  qx.Class.define("qxl.dialog.TabbedMultiColumnFormRenderer", {
    ...
    construct: function construct(form, tabInfo) {
      var _this = this;

      var render; // Save the render function, but prevent it from being called (by
      ...
      qxl.dialog.MultiColumnFormRenderer.constructor.call(this, form); // Replace the grid layout that the superclass instantiated, with
      // a VBox layout

      this._setLayout(new qx.ui.layout.VBox()); // Keep track of each of our tab pages


      this._pages = []; // Create the tabview for this renderer

      this._tabView = new qx.ui.tabview.TabView();

      this._add(this._tabView, {
        flex: 1
      }); // Add each of the requested pages to it


      tabInfo.forEach(function (info) {
        var page = new qx.ui.tabview.Page(info.name, info.icon);
        page.setLayout(info.layout || new qx.ui.layout.VBox());

        _this._tabView.add(page);
      ...
      this._tabView.addListener("changeSelection", function (e) {
        if (!_this.__P_425_0) {
          _this.fireDataEvent("changeTab", e.getData()[0]);
        }
      ...
      this._render(); // Make the tabview available via the form


      form.getTabViewInfo = function () {
        return {
          tabView: _this._tabView,
          pages: _this._pages
        };
      };
    },
    ...
    members: {
      ...
      addItems: function addItems(items, names, title) {
        ...
        var rowspan;
        var widget;
        var label; // Prevent changeTab event while we're in here
        ...
        if (title !== null) {
          this._pages.forEach(function (page) {
            // Switch to this page
            _this2._tabView.setValue(page); // Add the header to this page


            page.add(_this2._createHeader(title), {
              row: _this2._row++,
              column: 0,
              colSpan: 2
            });
        ...
        for (i = 0; i < items.length; i++) {
          ...
          item = items[i]; // If there is user data containing the page/row/column info, use it
          ...
          this._row = row;
          this._col = col; // Switch to the specified page
        ...
        this._tabView.setValue(page); // Re-enable changeTab events for when user selects a tab


        this.__P_425_0 = false;
      }
    }
  });
  qxl.dialog.TabbedMultiColumnFormRenderer.$$dbClassInfo = $$dbClassInfo;
})();

//# sourceMappingURL=TabbedMultiColumnFormRenderer.js.map?dt=1626057646894