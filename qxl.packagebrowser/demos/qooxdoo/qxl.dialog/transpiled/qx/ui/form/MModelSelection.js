  ...
  ************************************************************************ */
  ...
  qx.Mixin.define("qx.ui.form.MModelSelection", {
    construct: function construct() {
      ...
      this.__P_270_0 = new qx.data.Array();

      // listen to the changes
      this.__P_270_0.addListener("change", this.__P_270_1, this);
      this.addListener("changeSelection", this.__P_270_2, this);
    },
    events: {
      /**
    ...
    members: {
       ...
       */
      __P_270_1: function __P_270_1() {
        this.__P_270_3 = true;
        var selectables = this.getSelectables(true);
        var itemSelection = [];
        var modelSelection = this.__P_270_0.toArray();
      ...
      setModelSelection: function setModelSelection(modelSelection) {
        ...
        if (!modelSelection) {
          this.__P_270_0.removeAll();
          return;
        }
        // add the first two parameter
        modelSelection.unshift(this.__P_270_0.getLength()); // remove index
        modelSelection.unshift(0); // start index

        var returnArray = this.__P_270_0.splice.apply(this.__P_270_0, modelSelection);
        returnArray.dispose();
      }
    },
