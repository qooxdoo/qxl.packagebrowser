  ...
  qx.Class.define("qx.ui.form.validation.Manager", {
    ...
    members: {
      ...
      add(formItem, validator, context) {
        ...
        if (!this.__P_273_4(formItem)) {
          throw new Error("Added widget not supported.");
        }
        // check for the data type
        if (this.__P_273_5(formItem) && !formItem.getValue) {
      ...
      getItems() {
        var items = [];
        for (var i = 0; i < this.__P_273_0.length; i++) {
          items.push(this.__P_273_0[i].item);
        }
        return items;
      },
      /**
      ...
      validate() {
        var valid = true;
        this.__P_273_3 = true; // collaboration of all synchronous validations
        var items = [];

        // check all validators for the added form items
        for (var i = 0; i < this.__P_273_0.length; i++) {
          var formItem = this.__P_273_0[i].item;
          var validator = this.__P_273_0[i].validator;
          ...
          if (validator == null) {
            ...
            this.__P_273_3 = validatorResult && this.__P_273_3;
            continue;
          }
          var validatorResult = this._validateItem(this.__P_273_0[i], formItem.getValue());
        ...
        var formValid = this.__P_273_6(items);
        if (qx.lang.Type.isBoolean(formValid)) {
          this.__P_273_3 = formValid && this.__P_273_3;
        }
        valid = formValid && valid;
        this._setValid(valid);
        if (qx.lang.Object.isEmpty(this.__P_273_1)) {
      ...
      _validateRequired(formItem) {
        if (formItem.getRequired()) {
          ...
          if (this.__P_273_5(formItem)) {
            validatorResult = !!formItem.getSelection()[0];
          } else if (this.__P_273_7(formItem)) {
      ...
      _validateItem(dataEntry, value) {
        ...
        if (this.__P_273_8(validator)) {
          // used to check if all async validations are done
          this.__P_273_1[formItem.toHashCode()] = null;
      ...
      __P_273_6(items) {
        ...
        if (this.__P_273_8(formValidator)) {
          this.__P_273_1[this.toHashCode()] = null;
      ...
      _showToolTip(valid) {
        ...
        if (!valid) {
          var firstInvalid;
          for (var i = 0; i < this.__P_273_0.length; i++) {
            var item = this.__P_273_0[i].item;
            if (!item.isValid()) {
              firstInvalid = item;
      ...
      reset() {
        ...
        for (var i = 0; i < this.__P_273_0.length; i++) {
          var dataEntry = this.__P_273_0[i];
      ...
      setItemValid(formItem, valid) {
        // store the result
        this.__P_273_1[formItem.toHashCode()] = valid;
       ...
       */
      setFormValid(valid) {
        this.__P_273_1[this.toHashCode()] = valid;
        this.__P_273_9();
      },
      /**
       ...
       */
      __P_273_9() {
        var valid = this.__P_273_3;

        // check if all async validators are done
        for (var hash in this.__P_273_1) {
          var currentResult = this.__P_273_1[hash];
