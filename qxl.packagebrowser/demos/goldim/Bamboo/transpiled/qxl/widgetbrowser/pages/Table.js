  ...
  qx.Class.define("qxl.widgetbrowser.pages.Table", {
    ...
    members: {
      ...
      initWidgets: function initWidgets() {
        ...
        var table = this.__P_233_1();
        table.setFocusedCell(2, 5);
        widgets.push(table);
        this.add(table);
      },
      __P_233_1: function __P_233_1() {
        var rowData = this.__P_233_2(500);
      ...
      __P_233_2: function __P_233_2(rowCount) {
        var rowData = [];
        var now = new Date().getTime();
        var dateRange = 34560000000; // 400 days
        for (var row = 0; row < rowCount; row++) {
          var date = new Date(now + Math.random() * dateRange - dateRange / 2);
          rowData.push([this.__P_233_0++, Math.random() * 10000, date, Math.random() > 0.5]);
        }
        return rowData;
      }
    }
