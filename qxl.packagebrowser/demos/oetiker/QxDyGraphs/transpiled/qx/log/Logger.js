  ...
  qx.Bootstrap.define("qx.log.Logger", {
    statics: {
      ...
      */
      ...
      setTreshold: function setTreshold(value) {
        this.__P_97_1.setMaxMessages(value);
      },
      /**
      ...
      getTreshold: function getTreshold() {
        return this.__P_97_1.getMaxMessages();
      },
      /*
      ---------------------------------------------------------------------------
        APPENDER MANAGEMENT
      ...
      */
      ...
      register: function register(appender) {
        ...
        // Register appender
        var id = this.__P_97_5++;
        var appenderName = appender.appenderName || appender.classname;
        this.__P_97_2[id] = appender;
        this.__P_97_3[appenderName] = appender;
        appender.$$id = id;

        // Insert previous messages
        var entries = this.__P_97_1.getAllLogEvents();
        for (var i = 0, l = entries.length; i < l; i++) {
          var entry = entries[i];
          var appenders = this.__P_97_6(entry.loggerName, entry.level);
      ...
      unregister: function unregister(appender) {
        ...
        var appenderName = appender.appenderName || appender.classname;
        delete this.__P_97_3[appenderName];
        delete this.__P_97_2[id];
        delete appender.$$id;
      },
      /**
      ...
      */
      ...
      debug: function debug(object, message) {
        qx.log.Logger.__P_97_7("debug", arguments);
      },
      /**
      ...
      info: function info(object, message) {
        qx.log.Logger.__P_97_7("info", arguments);
      },
      /**
      ...
      warn: function warn(object, message) {
        qx.log.Logger.__P_97_7("warn", arguments);
      },
      /**
      ...
      error: function error(object, message) {
        qx.log.Logger.__P_97_7("error", arguments);
      },
      /**
      ...
      trace: function trace(object, message) {
        if (qx.log.Logger.isLoggerEnabled("trace", object)) {
          ...
          qx.log.Logger.__P_97_7("trace", args);
        }
      },
      /**
      ...
      */
      ...
      __P_97_1: new qx.log.appender.RingBuffer(50),
       ...
       */
      __P_97_10: function __P_97_10(object) {
        if (object) {
          if (object.classname) {
      ...
      isLoggerEnabled: function isLoggerEnabled(level, object) {
        var loggerName = this.__P_97_10(object);
        var appenders = this.__P_97_6(loggerName, level);
       ...
       */
      __P_97_7: function __P_97_7(level, args) {
        ...
        for (var i = start, l = args.length; i < l; i++) {
          items.push(this.__P_97_11(args[i], true));
        }
       ...
       */
      __P_97_6: function __P_97_6(className, level) {
        var levels = this.__P_97_8;

        // If no filters, then all appenders apply
        if (!this.__P_97_4.length) {
          // Check the default level
          if (levels[level] < levels[this.__P_97_0]) {
            return [];
          }
        ...
        var appenders = this.__P_97_9[cacheId];
        if (appenders !== undefined) {
          return appenders;
        }
        appenders = {};
        for (var i = 0; i < this.__P_97_4.length; i++) {
          var filter = this.__P_97_4[i];
          ...
          if (!filter.loggerMatch || filter.loggerMatch.test(className)) {
            if (filter.appenderName) {
              appenders[filter.appenderName] = this.__P_97_3[filter.appenderName];
            } else {
              return this.__P_97_9[cacheId] = this.__P_97_3;
            }
          }
        }
        return this.__P_97_9[cacheId] = appenders;
      },
      /**
       ...
       */
      __P_97_12: function __P_97_12(value) {
        if (value === undefined) {
          return "undefined";
        } else if (value === null) {
          return "null";
       ...
       * <code>trace</code>.
       */
      __P_97_11: function __P_97_11(value, deep) {
        var type = this.__P_97_12(value);
        var text = "unknown";
        var trace = [];
        switch (type) {
          case "null":
          ...
          case "array":
            if (deep) {
              ...
              for (var i = 0, l = value.length; i < l; i++) {
                ...
                text.push(this.__P_97_11(value[i], false));
              }
            } else {
              text = "[...(" + value.length + ")]";
            }
          ...
          case "map":
            if (deep) {
              ...
              for (var i = 0, l = sorted.length; i < l; i++) {
                ...
                key = sorted[i];
                temp = this.__P_97_11(value[key], false);
                temp.key = key;
                text.push(temp);
              }
            } else {
    ...
    defer: function defer(statics) {
      ...
      for (var i = 0; i < logs.length; i++) {
        statics.__P_97_7(logs[i][0], logs[i][1]);
      }
